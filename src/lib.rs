...
impl Card {
    ...
    pub fn supertypes(&self) -> Option<&[String]> {
        self.supertypes.as_ref().map_or(None, |v| Some(v.as_slice()))
    }
    pub fn types(&self) -> Option<&[String]> {
        self.types.as_ref().map_or(None, |v| Some(v.as_slice()))
    }
    pub fn subtypes(&self) -> Option<&[String]> {
        self.subtypes.as_ref().map_or(None, |v| Some(v.as_slice()))
    }
    ...
    pub fn power(&self) -> Option<&str> {
        match self.extra {
            ExtraInfo::PowerToughness(ref p, _) => Some(p.as_slice()),
            _                                   => None
        }
    }
    pub fn toughness(&self) -> Option<&str> {
        match self.extra {
            ExtraInfo::PowerToughness(_, ref t) => Some(t.as_slice()),
            _                                   => None
        }
    }
    pub fn power_toughness(&self) -> Option<(&str, &str)> {
        match self.extra {
            ExtraInfo::PowerToughness(ref p, ref t) => Some((p.as_slice(), t.as_slice())),
            _                                       => None
        }
    }
    pub fn starting_loyalty(&self) -> Option<u32> {
        match self.extra {
            ExtraInfo::StartingLoyalty(l) => Some(l),
            _                             => None
        }
    }

    // Functions on colors
    pub fn is_color(&self, col: Color) -> bool {
        self.color.as_ref().map_or(false, |v| v.iter().any(|c| *c == col))
    }
    pub fn is_noncolor(&self, col: Color) -> bool {
        self.color.as_ref().map_or(false, |v| v.iter().any(|c| *c != col))
    }
    pub fn is_colorless(&self) -> bool {
        self.color.as_ref().map_or(true, |v| v.len() == 0)
    }

    // Functions on types of various levels
    pub fn is_supertype(&self, ty: &str) -> bool {
        self.supertypes.as_ref().map_or(false, |v| v.iter().any(|t| t.as_slice() == ty))
    }
    pub fn is_nonsupertype(&self, ty: &str) -> bool {
        self.supertypes.as_ref().map_or(false, |v| v.iter().any(|t| t.as_slice() != ty))
    }
    pub fn is_type(&self, ty: &str) -> bool {
        self.types.as_ref().map_or(false, |v| v.iter().any(|t| t.as_slice() == ty))
    }
    pub fn is_nontype(&self, ty: &str) -> bool {
        self.types.as_ref().map_or(false, |v| v.iter().any(|t| t.as_slice() != ty))
    }
    pub fn is_subtype(&self, ty: &str) -> bool {
        self.subtypes.as_ref().map_or(false, |v| v.iter().any(|t| t.as_slice() == ty))
    }
    pub fn is_nonsubtype(&self, ty: &str) -> bool {
        self.subtypes.as_ref().map_or(false, |v| v.iter().any(|t| t.as_slice() != ty))
    }
}
